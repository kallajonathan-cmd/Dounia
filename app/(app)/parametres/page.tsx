  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg w-fit">
        {[
          { key: 'societe' as const, label: 'Entreprise', icon: <Building2 className="w-4 h-4" /> },
          { key: 'profil' as const, label: 'Mon profil', icon: <User className="w-4 h-4" /> },
        ].map(({ key, label, icon }) => (
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 max-w-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              { key: 'forme_juridique', label: 'Forme juridique', type: 'text', span: 1 },
              { key: 'siret', label: 'SIRET', type: 'text', span: 1 },
              { key: 'siren', label: 'SIREN', type: 'text', span: 1 },
              { key: 'capital', label: 'Capital (€)', type: 'number', span: 1 },
              { key: 'tva_intracommunautaire', label: 'TVA intracommunautaire', type: 'text', span: 1 },
              { key: 'ape', label: 'Code APE', type: 'text', span: 1 },
              { key: 'rcs', label: 'RCS', type: 'text', span: 1 },
              { key: 'telephone', label: 'Téléphone', type: 'tel', span: 1 },
              { key: 'email', label: 'Email', type: 'email', span: 1 },
              { key: 'site_web', label: 'Site web', type: 'url', span: 2 },
            ...
            <div className="md:col-span-2">
              ...
              <textarea rows={3} value={societeData.mention_legale || ''}
                onChange={(e) => setSocieteData({ ...societeData, mention_legale: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 max-w-md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={profileData.prenom}
                onChange={(e) => setProfileData({ ...profileData, prenom: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Nom</label>
              <input type="text" value={profileData.nom}
                onChange={(e) => setProfileData({ ...profileData, nom: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Téléphone</label>
              <input type="tel" value={profileData.telephone}
