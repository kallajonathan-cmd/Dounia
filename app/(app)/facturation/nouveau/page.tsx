...
function NouvelleFactureForm() {
  ...
  const [formData, setFormData] = useState({
    client_id: '', chantier_id: '', devis_id: devisId || '',
    type: 'facture', objet: '', taux_tva: 20, conditions_paiement: '',
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (devisId) {
        ...
        if (devis) {
          setFormData(f => ({ ...f, client_id: devis.client_id, devis_id: devis.id, objet: devis.objet, taux_tva: devis.taux_tva }))
          if (devis.devis_lignes?.length > 0) {
            setLignes(devis.devis_lignes.map((l: any, i: number) => ({
              position: i + 1, designation: l.designation, description: l.description || '',
              unite: l.unite || 'forfait', quantite: l.quantite, prix_unitaire_ht: l.prix_unitaire_ht,
              taux_tva: l.taux_tva, montant_ht: l.montant_ht,
            })))
          }
        }
      }
  ...
  const updateLigne = (index: number, field: string, value: any) => {
    ...
    if (field === 'quantite' || field === 'prix_unitaire_ht') nl[index].montant_ht = nl[index].quantite * nl[index].prix_unitaire_ht
    setLignes(nl)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data: facture, error: err } = await supabase.from('factures').insert({
      ...
      societe_id: profile?.societe_id, numero, statut: 'brouillon',
      montant_ht: montantHT, montant_tva: montantTVA, montant_ttc: montantTTC,
      montant_paye: 0, montant_restant: montantTTC, created_by: user?.id,
    }).select().single()
    if (err || !facture) { setError(err?.message || 'Erreur'); setLoading(false); return }
    await supabase.from('facture_lignes').insert(lignes.map((l, i) => ({ ...l, facture_id: facture.id, position: i + 1 })))
  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/facturation" className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><ArrowLeft className="w-5 h-5 text-gray-600" /></Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {lignes.map((l, i) => (
                  <tr key={i} className="border-t border-gray-50">
                    ...
                    <td className="px-3 py-2"><input type="text" value={l.unite} onChange={(e) => updateLigne(i, 'unite', e.target.value)} className="w-full border border-gray-200 rounded px-2 py-1 text-sm" /></td>
                    <td className="px-3 py-2"><input type="number" value={l.quantite} onChange={(e) => updateLigne(i, 'quantite', parseFloat(e.target.value) || 0)} className="w-full border border-gray-200 rounded px-2 py-1 text-sm text-right" /></td>
                    <td className="px-3 py-2"><input type="number" step="0.01" value={l.prix_unitaire_ht} onChange={(e) => updateLigne(i, 'prix_unitaire_ht', parseFloat(e.target.value) || 0)} className="w-full border border-gray-200 rounded px-2 py-1 text-sm text-right" /></td>
              ...
              <tfoot className="bg-gray-50">
                <tr><td colSpan={5} className="px-4 py-2 text-right text-sm font-medium">Total HT</td><td className="px-4 py-2 text-right font-bold">{montantHT.toFixed(2)} €</td><td></td></tr>
                <tr><td colSpan={5} className="px-4 py-2 text-right text-sm font-medium">TVA {formData.taux_tva}%</td><td className="px-4 py-2 text-right font-bold">{montantTVA.toFixed(2)} €</td><td></td></tr>
                <tr><td colSpan={5} className="px-4 py-2 text-right text-sm font-bold text-gray-900">Total TTC</td><td className="px-4 py-2 text-right font-bold text-orange-600">{montantTTC.toFixed(2)} €</td><td></td></tr>
              </tfoot>
            </table>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <button type="submit" disabled={loading}
            ...
            {loading && <Loader2 className="w-4 h-4 animate-spin" />}Créer la facture
          </button>
        </div>
      </form>
    </div>
