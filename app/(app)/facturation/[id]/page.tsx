      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-gray-500">Montant HT</span><span className="font-medium">{formatCurrency(facture.montant_ht)}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">TVA ({facture.taux_tva}%)</span><span className="font-medium">{formatCurrency(facture.montant_tva)}</span></div>
              <div className="flex justify-between pt-2 border-t border-gray-100 font-bold"><span>Total TTC</span><span>{formatCurrency(facture.montant_ttc)}</span></div>
              <div className="flex justify-between text-green-600"><span>Payé</span><span>{formatCurrency(facture.montant_paye)}</span></div>
              <div className="flex justify-between text-orange-600 font-bold"><span>Restant</span><span>{formatCurrency(facture.montant_restant)}</span></div>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100"><h3 className="font-semibold text-gray-900">Lignes</h3></div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
            <div className="px-5 py-4 border-b border-gray-100"><h3 className="font-semibold text-gray-900">Paiements ({paiements?.length || 0})</h3></div>
              ...
              {paiements && paiements.length > 0 ? paiements.map((p) => (
                <div key={p.id} className="px-5 py-3 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{p.mode}</p>
                  </div>
                  <p className="text-sm font-semibold text-green-600">{formatCurrency(p.montant)}</p>
                </div>
              )) : (
                <p className="px-5 py-6 text-sm text-gray-400 text-center">Aucun paiement</p>
              )}
            </div>
          </div>
        </div>
