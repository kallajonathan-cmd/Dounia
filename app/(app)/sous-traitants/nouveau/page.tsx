...
export default function NouveauSousTraitantPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '', telephone: '', contact_nom: '', specialites: [] as string[],
    kbis_date_validite: '', assurance_decennale_date_validite: '',
    assurance_rc_date_validite: '', notes: '',
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (err) { setError(err.message); setLoading(false) } else { router.push('/sous-traitants'); router.refresh() }
  }

  return (
    <div className="p-6 max-w-2xl">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/sous-traitants" className="p-2 hover:bg-gray-100 rounded-lg"><ArrowLeft className="w-5 h-5 text-gray-600" /></Link>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <input type="text" required value={formData.nom} onChange={(e) => setFormData({ ...formData, nom: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
          </div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">SIRET</label><input type="text" value={formData.siret} onChange={(e) => setFormData({ ...formData, siret: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Contact</label><input type="text" value={formData.contact_nom} onChange={(e) => setFormData({ ...formData, contact_nom: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" /></div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex gap-2 mb-2">
              ...
              <button type="button" onClick={addSpecialite} className="px-3 py-2 bg-orange-100 text-orange-700 rounded-lg hover:bg-orange-200">
            ...
            <div className="flex flex-wrap gap-1.5">
              {formData.specialites.map((s, i) => (
                <span key={i} className="flex items-center gap-1 text-xs bg-orange-50 text-orange-700 px-2 py-1 rounded-full">
                  {s}
                  <button type="button" onClick={() => setFormData({ ...formData, specialites: formData.specialites.filter((_, j) => j !== i) })}><X className="w-3 h-3" /></button>
                </span>
              ))}
            </div>
          </div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Validité KBIS</label><input type="date" value={formData.kbis_date_validite} onChange={(e) => setFormData({ ...formData, kbis_date_validite: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Validité décennale</label><input type="date" value={formData.assurance_decennale_date_validite} onChange={(e) => setFormData({ ...formData, assurance_decennale_date_validite: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" /></div>
        </div>
