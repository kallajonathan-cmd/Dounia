...
export default function ChantierMap({
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const center: [number, number] = initialLat && initialLng ? [initialLat, initialLng] : [46.5, 2.5]
      ...
      <div className="px-5 py-4 border-b border-gray-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {lat && lng && (
              <span className="text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded-full font-medium">Géolocalisé</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            {lat && lng && (
              <button onClick={removeCoords} className="flex items-center gap-1 text-xs text-red-500 hover:text-red-700 px-2 py-1.5 rounded-lg hover:bg-red-50 transition-colors">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-medium transition-colors ${
                clickMode ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`}
            >
        ...
        {message && (
          <div className={`mt-2 flex items-center gap-2 text-xs px-3 py-2 rounded-lg ${
            ...
            {status === 'error' ? <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" /> : <CheckCircle className="w-3.5 h-3.5 flex-shrink-0" />}
            {message}
          </div>
        )}
        {addressLabel && (
          <p className="mt-1.5 text-xs text-gray-400 flex items-center gap-1">
            <MapPin className="w-3 h-3" />{addressLabel}
          </p>
        )}
        {lat && lng && (
          <p className="mt-1 text-xs text-gray-400 font-mono">{lat.toFixed(6)}, {lng.toFixed(6)}</p>
        )}
      </div>
