...
import { ArrowLeft, Edit, MapPin, Calendar, TrendingUp } from 'lucide-react'
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href={`/chantiers/${params.id}/modifier`}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        ...
        <div className="w-full bg-gray-100 rounded-full h-3">
          <div className="bg-gradient-to-r from-orange-400 to-orange-600 h-3 rounded-full transition-all"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-3">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-gray-500">Début prévu</span><span>{formatDate(chantier.date_debut_prevue)}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Fin prévue</span><span>{formatDate(chantier.date_fin_prevue)}</span></div>
              {chantier.date_debut_reelle && <div className="flex justify-between"><span className="text-gray-500">Début réel</span><span className="text-green-600">{formatDate(chantier.date_debut_reelle)}</span></div>}
              {chantier.date_fin_reelle && <div className="flex justify-between"><span className="text-gray-500">Fin réelle</span><span className="text-green-600">{formatDate(chantier.date_fin_reelle)}</span></div>}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-3">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-gray-500">Budget HT</span><span className="font-medium">{chantier.budget_ht ? formatCurrency(chantier.budget_ht) : '-'}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Coût réel HT</span><span className="font-medium">{chantier.cout_reel_ht ? formatCurrency(chantier.cout_reel_ht) : '-'}</span></div>
              {chantier.marge_prevue_pct && <div className="flex justify-between"><span className="text-gray-500">Marge prévue</span><span className="font-medium text-green-600">{chantier.marge_prevue_pct}%</span></div>}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
            <div className="px-5 py-4 border-b border-gray-100"><h3 className="font-semibold text-gray-900">Lots de travaux</h3></div>
              ...
              {lots && lots.length > 0 ? lots.map((lot) => (
                <div key={lot.id} className="px-5 py-3">
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-1.5">
                    <div className="h-1.5 rounded-full" style={{ width: `${lot.avancement_pct}%`, backgroundColor: lot.couleur || '#f97316' }} />
                  </div>
                  {(lot.date_debut || lot.date_fin) && (
                    <p className="text-xs text-gray-400 mt-1">{formatDate(lot.date_debut)} → {formatDate(lot.date_fin)}</p>
                  )}
                </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
            <div className="px-5 py-4 border-b border-gray-100"><h3 className="font-semibold text-gray-900">Journal de chantier</h3></div>
            <div className="divide-y divide-gray-50">
              {journal && journal.length > 0 ? journal.map((j) => (
                <div key={j.id} className="px-5 py-3">
                  ...
                  {j.travaux_realises && <p className="text-xs text-gray-600 line-clamp-2">{j.travaux_realises}</p>}
                  {j.conditions_meteo && <p className="text-xs text-blue-500 mt-1">☁ {j.conditions_meteo} — {j.temperature_matin}°C</p>}
                </div>
