  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="flex items-center gap-4 mb-6">
        <Link href={`/chantiers/${params.id}`} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="number" min="0" max="100" value={formData.avancement_pct || 0}
              onChange={(e) => setFormData({ ...formData, avancement_pct: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="date" value={formData.date_debut_reelle || ''} onChange={(e) => setFormData({ ...formData, date_debut_reelle: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Fin réelle</label>
            <input type="date" value={formData.date_fin_reelle || ''} onChange={(e) => setFormData({ ...formData, date_fin_reelle: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Coût réel HT (€)</label>
            <input type="number" step="0.01" value={formData.cout_reel_ht || ''} onChange={(e) => setFormData({ ...formData, cout_reel_ht: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
          </div>
          <div className="md:col-span-2">
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
          <Link href={`/chantiers/${params.id}`} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-900">Annuler</Link>
