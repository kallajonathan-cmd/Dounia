...
export default async function MateriauxPage() {
  ...
  const statusColors: Record<string, string> = {
    ...
    confirmee: 'bg-purple-100 text-purple-700', livree_partiel: 'bg-yellow-100 text-yellow-700',
    livree: 'bg-green-100 text-green-700', annulee: 'bg-red-100 text-red-600',
  }
  const statusLabels: Record<string, string> = {
    brouillon: 'Brouillon', envoyee: 'Envoyée', confirmee: 'Confirmée',
    livree_partiel: 'Liv. partielle', livree: 'Livrée', annulee: 'Annulée',
  }

  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Matériaux &amp; Commandes</h1><p className="text-gray-500 text-sm mt-1">{commandes?.length || 0} commandes</p></div>
        <Link href="/materiaux/nouvelle-commande"
            ...
            <tbody className="divide-y divide-gray-50">
              {commandes && commandes.length > 0 ? commandes.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm font-semibold text-right">{c.montant_ttc ? formatCurrency(c.montant_ttc) : '-'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-medium px-2 py-1 rounded-full ${statusColors[c.statut] || 'bg-gray-100'}`}>{statusLabels[c.statut] || c.statut}</span>
                  </td>
                </tr>
              )) : (
                <tr><td colSpan={7} className="px-4 py-12 text-center text-sm text-gray-400">Aucune commande.</td></tr>
              )}
            </tbody>
          </table>
        </div>
