...
export default function NouvelleCommandePage() {
  ...
  const updateLigne = (i: number, field: string, value: any) => {
    ...
    if (field === 'quantite_commandee' || field === 'prix_unitaire_ht' || field === 'remise_pct') {
      const q = nl[i].quantite_commandee
      const p = nl[i].prix_unitaire_ht
      const r = nl[i].remise_pct || 0
      nl[i].montant_ht = q * p * (1 - r / 100)
    }
    setLignes(nl)
  }
  ...
  const taux_tva = 20
  const montantTTC = montantHT * (1 + taux_tva / 100)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await supabase.from('commande_lignes').insert(
      lignes.map((l, i) => ({ ...l, commande_id: commande.id, position: i + 1, quantite_livree: 0 }))
    )
  ...
  return (
    <div className="p-6 max-w-4xl">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/materiaux" className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <input type="text" value={formData.adresse_livraison} onChange={(e) => setFormData({ ...formData, adresse_livraison: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <button type="button"
              onClick={() => setLignes([...lignes, { position: lignes.length + 1, reference_produit: '', designation: '', unite: 'u', quantite_commandee: 1, prix_unitaire_ht: 0, remise_pct: 0, montant_ht: 0 }])}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="text-left text-xs text-gray-500 px-3 py-2">Réf.</th>
                  ...
                  <th className="text-right text-xs text-gray-500 px-3 py-2">Remise %</th>
                ...
                {lignes.map((l, i) => (
                  <tr key={i} className="border-t border-gray-50">
                    <td className="px-3 py-2">
                      <input type="text" value={l.reference_produit} onChange={(e) => updateLigne(i, 'reference_produit', e.target.value)}
                    ...
                    <td className="px-3 py-2">
                      <input type="number" value={l.quantite_commandee} onChange={(e) => updateLigne(i, 'quantite_commandee', parseFloat(e.target.value) || 0)}
                    ...
                    <td className="px-3 py-2">
                      <input type="number" min="0" max="100" value={l.remise_pct} onChange={(e) => updateLigne(i, 'remise_pct', parseFloat(e.target.value) || 0)}
                        className="w-20 border border-gray-200 rounded px-2 py-1 text-sm text-right" />
                    </td>
              ...
              <tfoot className="bg-gray-50">
                <tr>
                  <td colSpan={6} className="px-4 py-2 text-right text-sm font-medium">Total HT</td>
                  <td className="px-4 py-2 text-right font-bold">{montantHT.toFixed(2)} €</td>
                  <td></td>
                </tr>
                <tr>
                  <td colSpan={6} className="px-4 py-2 text-right text-sm font-medium">TVA 20%</td>
                  <td className="px-4 py-2 text-right font-bold">{(montantHT * 0.2).toFixed(2)} €</td>
                  <td></td>
                </tr>
                <tr>
                  <td colSpan={6} className="px-4 py-2 text-right text-sm font-bold">Total TTC</td>
                  <td className="px-4 py-2 text-right font-bold text-orange-600">{montantTTC.toFixed(2)} €</td>
                  <td></td>
                </tr>
              </tfoot>
            </table>
