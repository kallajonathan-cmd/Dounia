...
const TYPES_ASSURANCE = ['Responsabilité Civile', 'Décennale', 'Dommages Ouvrage', 'Multirisque', 'Flotte Auto', 'Matériel', 'Autre']
...
export default function NouvelleAssurancePage() {
  ...
  const [formData, setFormData] = useState({
    type: '', compagnie: '', numero_contrat: '', objet: '',
    montant_prime_annuelle: '', date_debut: '', date_echeance: '',
    date_renouvellement: '', tacite_reconduction: false,
    alerte_60j: true, alerte_30j: true, alerte_15j: true, notes: '',
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (err) { setError(err.message); setLoading(false) } else { router.push('/assurances'); router.refresh() }
  }

  return (
    <div className="p-6 max-w-2xl">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/assurances" className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><ArrowLeft className="w-5 h-5 text-gray-600" /></Link>
