...
export default async function AssurancesPage() {
  ...
  const { data: assurances } = await supabase.from('assurances').select('*').eq('societe_id', profile?.societe_id || '').order('date_echeance')
    ...
    <div className="p-6 space-y-6">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              {expiringSoon.map(a => (
                <p key={a.id} className="text-sm text-yellow-700 mt-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {assurances && assurances.length > 0 ? assurances.map((a) => {
          ...
          return (
            <div key={a.id} className={`bg-white rounded-xl border shadow-sm p-5 ${isExpired ? 'border-red-200' : isExpiringSoon ? 'border-yellow-200' : 'border-gray-100'}`}>
              <div className="flex items-start gap-3 mb-3">
                ...
                <div><h3 className="font-semibold text-gray-900">{a.type}</h3><p className="text-sm text-gray-500">{a.compagnie}</p></div>
              </div>
              <div className="space-y-1.5 text-sm">
                {a.numero_contrat && <p className="text-gray-500">N° {a.numero_contrat}</p>}
                {a.montant_prime_annuelle && <p className="font-medium text-gray-900">{formatCurrency(a.montant_prime_annuelle)}/an</p>}
              </div>
              <div className="mt-3 pt-3 border-t border-gray-100 grid grid-cols-2 gap-2 text-xs">
                <div><p className="text-gray-400">Début</p><p className="font-medium text-gray-700">{formatDate(a.date_debut)}</p></div>
                <div>
                  ...
                  <p className={`font-medium ${isExpired ? 'text-red-600' : isExpiringSoon ? 'text-yellow-600' : 'text-gray-700'}`}>
                    {formatDate(a.date_echeance)}{isExpired ? ' ⚠ Expirée' : isExpiringSoon ? ' ⚡ Bientôt' : ''}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="col-span-3 bg-white rounded-xl border border-gray-100 shadow-sm p-12 text-center">
            ...
            <Link href="/assurances/nouveau" className="mt-2 inline-block text-sm text-orange-600 hover:underline">Ajouter la première</Link>
          </div>
        )}
      </div>
    </div>
