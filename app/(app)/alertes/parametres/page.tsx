...
export default function AlertesParametresPage() {
  ...
  const [settings, setSettings] = useState({
    ...
    alertes_email: true,
    alerte_devis_expiration: true,
    alerte_facture_echeance: true,
    alerte_assurance_expiration: true,
    alerte_titre_sejour_expiration: true,
  })
  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/alertes" className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
          ...
          <div className="space-y-3">
            {[
              { key: 'alertes_whatsapp', label: 'Recevoir les alertes par WhatsApp' },
              { key: 'alertes_email', label: 'Recevoir les alertes par email' },
            ].map(({ key, label }) => (
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
          ...
          <div className="space-y-3">
            {[
              { key: 'alerte_devis_expiration', label: 'Devis arrivant à expiration' },
              { key: 'alerte_facture_echeance', label: 'Factures arrivant à échéance' },
              { key: 'alerte_assurance_expiration', label: 'Assurances à renouveler' },
              { key: 'alerte_titre_sejour_expiration', label: 'Titres de séjour expirant' },
            ].map(({ key, label }) => (
              <label key={key} className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
