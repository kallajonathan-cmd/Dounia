...
export default async function AlertesPage() {
  ...
  const { data: alertes } = await supabase.from('alertes').select('*').eq('societe_id', profile?.societe_id || '').order('created_at', { ascending: false }).limit(100)
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{nonLues > 0 ? `${nonLues} alerte(s) non lue(s)` : 'Toutes les alertes sont lues'}</p>
        </div>
        <div className="flex items-center gap-2">
          {nonLues > 0 && <MarkAllReadButton societeId={profile?.societe_id} />}
          <Link href="/alertes/parametres" className="flex items-center gap-2 border border-gray-300 hover:bg-gray-50 text-gray-700 px-4 py-2.5 rounded-lg text-sm font-medium">
      ...
      <div className="space-y-2">
        {alertes && alertes.length > 0 ? alertes.map((a) => (
          <div key={a.id} className={`flex items-start gap-4 p-4 rounded-xl border transition-colors ${!a.lue ? 'bg-white border-gray-200 shadow-sm' : 'bg-gray-50 border-gray-100'}`}>
            <div className="mt-0.5 flex-shrink-0">{iconMap[a.niveau] || <Bell className="w-4 h-4 text-gray-400" />}</div>
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {!a.lue && <span className="w-2 h-2 bg-orange-500 rounded-full" />}
                  <span className="text-xs text-gray-400">{formatDateTime(a.created_at)}</span>
                </div>
              </div>
              <div className="flex items-center gap-3 mt-1">
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${getNiveauAlerteColor(a.niveau)}`}>{a.niveau}</span>
