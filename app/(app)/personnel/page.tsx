...
export default async function PersonnelPage() {
  ...
  const { data: personnel } = await supabase.from('personnel').select('*').eq('societe_id', profile?.societe_id || '').order('nom')

  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Personnel</h1><p className="text-gray-500 text-sm mt-1">{personnel?.length || 0} employés</p></div>
        <Link href="/personnel/nouveau" className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition-colors">
              ...
              <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                    ...
                    p.statut === 'conge' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500'
                  }`}>{p.statut === 'actif' ? 'Actif' : p.statut === 'conge' ? 'En congé' : 'Inactif'}</span>
                </td>
                ...
                <td className="px-4 py-3">
                  {p.date_validite_titre ? (
                    ...
                    }`}>{new Date(p.date_validite_titre) < new Date() ? 'Expiré' : formatDate(p.date_validite_titre)}</span>
                  ) : <span className="text-xs text-gray-400">-</span>}
                </td>
              </tr>
            )) : (
              <tr><td colSpan={5} className="px-4 py-12 text-center text-sm text-gray-400">Aucun employé.</td></tr>
            )}
          </tbody>
        </table>
      </div>
