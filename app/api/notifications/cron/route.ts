...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Check devis expiring
    const { data: devisExpirants } = await supabase
      .from('devis')
      .select('*, societes:societe_id(id), clients(nom)')
      .eq('statut', 'envoye')
      .lte('date_validite', in30Days)
    ...
    // Insert all alertes (avoid duplicates by checking recent ones)
    if (alertes.length > 0) {
      // Insert new alerts
    ...
    return NextResponse.json({
      success: true,
      alertes_created: alertes.length,
